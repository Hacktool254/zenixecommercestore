...
- [ ] Create Next.js 15 app with TypeScript and App Router: `npx create-next-app@latest . --typescript --tailwind --app --eslint --src-dir=false --import-alias="@/*"`
- [ ] Verify project runs: `npm run dev`
- [ ] Configure `next.config.ts` — add Cloudinary domain to `images.remotePatterns`
- [ ] Set up `tsconfig.json` strict mode

### 1.2 Install All Dependencies
- [ ] Install production packages: `npm install convex @convex-dev/auth zustand @tanstack/react-query react-hook-form @hookform/resolvers zod lucide-react sonner clsx tailwind-merge class-variance-authority next-cloudinary cloudinary`
- [ ] Install Radix UI packages: `npm install @radix-ui/react-dialog @radix-ui/react-dropdown-menu @radix-ui/react-select @radix-ui/react-slider @radix-ui/react-tabs @radix-ui/react-switch @radix-ui/react-avatar`
- [ ] Install dev packages: `npm install -D prettier prettier-plugin-tailwindcss husky lint-staged`

### 1.3 Tailwind CSS v4 Setup
- [ ] Configure `globals.css` with CSS variables for the full Zenix color system (all tokens from design.md)
- [ ] Add Space Grotesk + Inter fonts via `next/font/google`
- [ ] Configure Tailwind theme extension with Zenix color tokens

### 1.4 shadcn/ui Setup
- [ ] Initialize shadcn: `npx shadcn@latest init` — choose dark theme, CSS variables
- [ ] Add base components: `npx shadcn@latest add button badge input label select sheet dialog tabs skeleton avatar dropdown-menu`
- [ ] Customize component styles to match Zenix design system

### 1.5 Convex Setup
- [ ] Create Convex account and project at convex.dev
- [ ] Initialize Convex in project: `npx convex dev`
- [ ] Set up `NEXT_PUBLIC_CONVEX_URL` in `.env.local`
- [ ] Wrap app with `ConvexClientProvider` in `app/layout.tsx`

### 1.6 Convex Auth Setup
- [ ] Install and configure Convex Auth: follow `@convex-dev/auth` docs
- [ ] Set `JWKS_PRIVATE_KEY` and `SITE_URL` in env
- [ ] Configure email/password provider
- [ ] Test signup and login flow

### 1.7 Project Structure
- [ ] Create all folders: `components/`, `lib/`, `stores/`, `types/`, `convex/`
- [ ] Create `lib/utils.ts` with `cn()` helper (clsx + tailwind-merge)
- [ ] Set up ESLint + Prettier config with `prettier-plugin-tailwindcss`
- [ ] Set up Husky pre-commit hook with lint-staged

### 1.8 Environment Variables
- [ ] Create `.env.local` with all variables from techstack.md
- [ ] Create `.env.example` with placeholder values (commit this, not `.env.local`)
